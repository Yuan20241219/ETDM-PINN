...
class E_pinn():
    ...
    def train(self, load_iter, epochs,result_indx,fun_V2F):
        self.mean=load_iter.mean
        self.var=load_iter.var
        kern_A=self._get_kernA(self.A, result_indx)
        scl_model=SCL(1, len(result_indx), kern_A).to("cuda")
        train_loss = []
        for epoch in range(epochs):
            for batch in load_iter:
                self.optimizer.zero_grad()
                p = self.lstm_model(batch.to("cuda"))
                p_total = torch.cat((batch[:, :, 0].unsqueeze(2).to("cuda"), p), dim=2)
                P_total = torch.cat(
    ...
    def predict(self,data):
        ...
        batch_data=next(SeqDataIter(data,len(data),self.n_features,mean=self.mean,var=self.var))
...
if __name__ == '__main__':
    ...
    train_data = pd.read_csv("地震加速度.csv").iloc[1:181, 1:]
    vali_data=pd.read_csv("地震加速度.csv").iloc[181, 1:]
    batch_size = 180
    n_features = 1
    ...
    train_loss = e_pinn_model.train(load_iter, epochs, result_indx,fun_V2F=V2Fnl)
